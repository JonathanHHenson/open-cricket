...
from labeljudge.integrations import as_runnable, chat_runnable, graph_node, ProbabilityUnavailableError
...
class ToyBackend:
    ...
    def next_logprobs(self, prompt, prefix, allowed):
        probabilities = {1: .6, 2: .3} if not prefix else {0: .5}
...
class FakeChat:
    ...
    def invoke(self, messages, config=None):
        ...
        entries = [{"token": "A", "logprob": math.log(.6)}]
        if not self.missing:
            entries.append({"token": "B", "logprob": math.log(.3)})
        return SimpleNamespace(response_metadata={"logprobs": {"content": [{"top_logprobs": entries}]}})
...
class IntegrationTests(unittest.TestCase):
    ...
    def test_real_langgraph_state_and_conditional_routing(self):
        ...
        g.add_conditional_edges("judge", lambda state: state["classification"]["answer"],
                               {"billing": "billing", "other": "other"})
        g.add_edge("billing", END)
        g.add_edge("other", END)
        graph = g.compile()
        for result in [graph.invoke({**VALUE, "marker": "preserve me"}),
