...
def main():
    ...
    clients = {'original': LocalClient(model=args.model, backend=Uncached()),
    ...
    for repeat in range(args.repeats + 1):
        ...
        for name in order:
            ...
            results[name] = clients[name].system_one(
                state=message, questions={'route': {'type': 'choice',
    ...
    difference = max(abs(p - original[label]) for label, p in
                     results['optimized']['answers']['route']['probabilities'].items())
    print(json.dumps({
