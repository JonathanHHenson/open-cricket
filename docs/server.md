...
| `OPEN_CRICKET_MODE` | `answer_codes` | Case-insensitive `A–Z`, `0–9`, then longer codes; `sequence` or `constrained` for full labels |
...
`answer_codes` combines all letter-case variants per candidate, scores single-token
sets in one pass, and scores longer codes with EOS through the cached trie, subject
to context and memory limits. It preserves original
